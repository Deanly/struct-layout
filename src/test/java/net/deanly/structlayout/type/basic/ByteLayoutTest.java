...
class ByteLayoutTest {
    ...
    void testDefaultConstructor() {
        // 기본 생성자 테스트
        ByteLayout layout = new ByteLayout();
    ...
    void testParameterizedConstructor() {
        ...
        ByteLayout layout = new ByteLayout(testProperty);
        assertNotNull(layout);
    ...
    public static class ByteStruct {
        @StructField(order = 1, dataType = DataType.BYTE)
        private byte singleByte; // 단일 byte 필드

        @SequenceField(order = 2, lengthType = DataType.UINT8, elementType = DataType.BYTE)
