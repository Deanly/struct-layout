...
public class StructFieldTypeCompatibilityDecodingTest {
    ...
    @Test
    public void testSequenceFieldDecoding() {
    ...
    public static class SimpleStruct {
        @StructField(order = 1, dataType = DataType.UINT8) public short uint8Field;
        @StructField(order = 2, dataType = DataType.INT8) public byte int8Field;
        @StructField(order = 3, dataType = DataType.INT8) public byte int8NegativeField;
        @StructField(order = 4, dataType = DataType.UINT8) public byte uint8SmallField;
    }

    // 2. 호환 가능한 구조체
    public static class CompatibleStruct {
        @StructField(order = 1, dataType = DataType.UINT8) public int uint8AsInt;
        @StructField(order = 2, dataType = DataType.UINT16_BE) public long uint16AsLong;
        @StructField(order = 3, dataType = DataType.FLOAT32_LE) public double float32AsDouble;
    }

    // 3. 부적합한 구조체
    public static class IncompatibleStruct {
        @StructField(order = 1, dataType = DataType.UINT8) public long uint8AsLong;
        @StructField(order = 2, dataType = DataType.FLOAT32_LE) public short floatField;
    }

    // 4. 시퀀스 포함 구조체
    public static class SequenceStruct {
        @SequenceField(order = 1, lengthType = DataType.UINT8, elementType = DataType.UINT8)
    ...
    public static class ComplexStruct {
        @StructField(order = 1, dataType = DataType.UINT8) public short uint8Field;
        @StructField(order = 2, dataType = DataType.INT8) public int int8Field;
        @SequenceField(order = 3, lengthType = DataType.UINT8, elementType = DataType.UINT8)
        public List<Integer> sequenceField;
        @CustomLayoutField(order = 4, layout = CustomStringLayout.class) public String customField;
    }

    // 테스트용 CustomLayoutField
    public static class CustomStringLayout extends Layout<String> {
        public CustomStringLayout() { super(4); }
        @Override
